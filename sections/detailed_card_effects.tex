...
\begin{expansion}{stretchgoals2}
  ...
  \setlength\columnsep{1em}
  \begin{wrapfigure}{r}{0.2\linewidth}
    \includegraphics[width=\linewidth]{\images/land-mine-token.png}\vspace*{1em}
    \includegraphics[width=\linewidth]{\images/land-mine-token-0.png}\vspace*{1em}
		\includegraphics[width=\linewidth]{\images/land-mine-token-2.png}
  \end{wrapfigure}
	Depending on the \svg{empower} used, take 2, 4 or 6 \textbf{Land Mine tokens} from the same set  (blue or red).
  Half of them are empty and half of them have +2 \svg{damage} symbol.
...
\begin{expansion}{cove}
  ...
  \begin{wrapfigure}{r}{0.2\linewidth}
	...
	\end{wrapfigure}
  The Few Sorceresses Unit can use its \svg{unit_other} abilty to place a \textbf{Weakness token} on any Unit.
  A pack Sorceresses deal that token automatically to their defending unit because of their \svg{unit_attack} ability.
...
\begin{expansion}{cove}
  ...
  \begin{wrapfigure}{r}{0.2\linewidth}
	...
	\end{wrapfigure}
  When you cast the Clone spell, choose an allied unit (depending on the \svg{empower} used) and an adjacent empty space on the Combat board to that unit.
  ...
  The token on the empty space represents the Clone.
	It has the same statistics and special abilities as the original unit, but its \svg{health_points} is only 1 and its \svg{defense} is.
  Additionally, its defense cannot be increased by any effects from cards and other units. %what is with other statistics?
	If the Cloned unit is attacked or takes even 1 \svg{damage} from a \svg{spellpower}, it perishes.
