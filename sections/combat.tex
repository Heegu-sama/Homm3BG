...
\pl{
  \addsection{Walka}{\skills/attack.png}
}

\begin{multicols}{2}

\en{\hypertarget{Combat}{Combat}\index{Combat} with \textbf{Neutral Units} starts when a Hero moves to an unvisited Field with a roman numeral, signifying the \hyperlink{Difficulty}{type and number} of Neutral Units guarding that Field.}
\pl{}

\en{Combat with \textbf{another player} can start in two ways:}
\pl{}
\begin{itemize}
  \en{\item You move into any Field containing one of their Heroes.}
  \pl{\item}
  \en{\item You move into a Town or Settlement owned by them.}
  \pl{\item}
\end{itemize}
\en{Players are able to start multiple Combats during their Turn.}
\pl{}

\en{\subsection*{\hypertarget{Combatsetup}{Combat Setup}}}
\pl{}
...
\begin{itemize}
  ...
  \pl{\item}
  \en{\item Check the \hyperlink{Difficulty}{\textbf{Difficulty Table}} and draw the corresponding number of Neutral Unit cards from their Decks.}
  \pl{\item}
  \en{\item The Neutral Units are placed differently depending on the Game Mode:}
  \pl{\item}
  \en{
  ...
  \pl{\item}
\end{itemize}
\en{Unit setup when fighting \textbf{other players}:}
\pl{}
\begin{itemize}[wide]
  \en{\item The attacking player places up to 5 Units on their chosen side of the Combat Board, followed by the defender.}
  \pl{\item}
  \en{\item If the Combat takes place in a Town with a Citadel, the defender adds the \hyperlink{Walls}{Wall, Gate and Arrow Tower} cards after placing their Units.}
  \pl{\item}
\end{itemize}
\vfill
\begin{center}
...
\pl{
  \subsection*{\hypertarget{Terminologia Walki}{Terminologia Walki}}
}
\en{\textbf{Attacking Player} – The player who started the Combat.\par}
\pl{}
\en{\textbf{Defending Player} – The player whom Combat was started against.\par}
\pl{}
\en{\textbf{Activation} – A Unit Activates when it is next in the Initiative order.\par}
\pl{}
\en{\textbf{Adjacent Unit} – A Unit is directly adjacent to another if it is one space away in a cardinal direction (nondiagonal).\par}
\pl{}
\en{\textbf{Combat Round} – A full cycle of all Units of each player being Activated.\par}
\pl{}
\en{
...
\note{5}{
  \en{Combats against Azure \includesvg[height=10px]{\svgs/azure-note.svg} Units, other players, or \hyperlink{AIrules}{AI Heroes} have no time limit.}
  \pl{}
}
...
\pl{
  \subsection*{\hypertarget{Mury}{Obrona Miasta z Cytadelą}}\index{Obrona Miasta}
}

\begin{center}
  \includegraphics[width=0.6\linewidth]{\cards/arrow_tower.png}
\end{center}
\en{The defender also gains the Arrow Tower\index{Arrow Tower} Unit Card which is placed next to the Combat Board.}
\pl{}
...
\begin{figure*}[h]
  ...
  \begin{minipage}[t]{0.415\textwidth}
    ...
    \caption[halberdiers unprotected]{
      \en{
...
\en{Structure of a Combat Round:}
\pl{}
\begin{itemize}
  \en{\item Players Activate their Units in descending order of Unit \hyperlink{Initiative}{Initiative}. \textbf{If there's a tie}, alternate between attackers and defenders starting with an attacker.}
  \pl{\item}
  \en{\item When a Unit Activates, place a Faction Cube on it to indicate it has been Activated this Combat Round.}
  \pl{\item}
  \en{\item Activated Units may move and attack according to their \hyperlink{Unittype}{type}. Neutral Units controlled by an opposing player must always attack if possible.}
  \pl{\item}
  \en{
  ...
  \pl{\item}
  \en{\item Before a Unit attacks, both players may \hyperlink{CombatCards}{play Cards}. Cards are resolved in the order in which players decide to play them.}
  \pl{\item}
  \en{
  ...
  \pl{\item}
  \en{\item If the defending Unit was adjacent to the attacker, it \hyperlink{Retaliate}{retaliates} if it hasn't done so this Round.}
  \pl{\item}
  \en{
...
\pl{}
\note{4}{
  \en{You cannot surrender when defending a Town.}
  \pl{}
}\par
\en{
