...
When you defend your besieged Town and it \textbf{doesn't have} a Citadel, Combat happens just like on any other Field.
Defending a Settlement is the same as defending a Town without a Citadel.

\vspace{0.5em}
\note{3.5}{Note: you cannot surrender when defending your besieged Faction Town.}\par
...
But if there are many valid targets, it cannot attack the nearest one, so it will attack the one that is closest to perishing (the smallest difference between \svg{health_points} and current \svg{damage}).
...
Defending Units standing on their own side and \textbf{in the same column} as an undestroyed Wall or a Gate gain protection from \svg{unit_ranged} attacks.
