...
\hypertarget{Combat}{Combat}\index{Combat} with \textbf{Neutral Units} starts when a Hero moves to an \textbf{unvisited} Field with a roman numeral, signifying the \hyperlink{Difficulty Table}{type and number} of Neutral Units guarding that Field.
...
\textbf{\hypertarget{Paralysis}{Paralysis}}\index{Paralysis} \includesvg[height=10px]{\svgs/paralysis.svg} – Some effects place a Paralysis Token on Units.
...
\textbf{\hypertarget{Defend}{Defend}} \includesvg[height=10px]{\svgs/defense.svg} – Units may choose to gain a Defense Token and end the Activation instead of attacking.
...
\subsection*{\hypertarget{combatround}{Combat Round Structure}}
...
The player is considered to have beaten the Neutral Units by default and gains no rewards from the Combat itself, then proceeds to Visit the Field.
