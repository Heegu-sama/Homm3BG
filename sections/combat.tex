...
\pagetarget{Combat}{Combat}\index{Combat} is resolved on the separate Combat Board.
Combat with \textbf{Neutral Units} starts when a Hero moves to an unvisited Field with a roman numeral, which signifies the \pagelink{Difficulty}{type and number} of Neutral Units guarding that Field.
...
\begin{itemize}
  ...
  \item Check the \pagelink{Difficulty}{\textbf{Difficulty Table}} and draw the corresponding number of Neutral Unit cards from their Decks.
...
Units must be placed in decreasing Initiative\index{Initiative} order.
...
\textbf{\pagetarget{Retaliate}{Retaliation Attack}} – If a Unit survives an attack by an adjacent Unit, it performs an attack back on that Unit.
Each Unit can perform only 1 Retaliation Attack\index{Retaliation Attack} per Combat Round.
...
\textbf{\pagetarget{Defend}{Defend}} \includesvg[height=10px]{\svgs/defense.svg} – Units may choose to gain a Defense Token instead of attacking.
...
If the player cannot win the Combat before the end of the current Combat Round, they must either \pagelink{Endcombat}{Retreat}\index{Retreat} or spend 1 MP from the Hero that started the Combat in order to play another Combat Round.\par
\note{5}{Combat against Azure \includesvg[height=10px]{\svgs/azure-note.svg} Units, other players or \pagelink{AIrules}{AI Heroes} have no time limit.}
...
The Wall and Gate\index{Walls and Gate} cards can be destroyed by any adjacent \includesvg[height=10px]{\svgs/unit_ground.svg} or \includesvg[height=10px]{\svgs/unit_flying.svg} Unit's attack.\par
\note{8}{Attacks against Walls are always successful and they do not follow the normal framework for attacking other Units.
  Do not roll the Combat Die, activate any \includesvg[height=10px]{\svgs/unit_attack-note.svg} Abilities or play any \includesvg[height=10px]{\svgs/instant-note.svg} Cards during them.
}\par
...
\begin{figure*}[h]
  ...
  \caption[halberdiers protected]{\textit{Halberdiers are behind a non-destroyed Gate, they \textbf{are protected} when attacked from behind the Wall line.
    The attack damage of Evil Eyes is \textbf{reduced \includesvg[height=10px]{\svgs/unit_ranged.svg} by 1}.}}
  \end{minipage}
  \hfill%
  \begin{minipage}[t]{0.415\textwidth}
    \centering
    \includegraphics[width=\linewidth]{\examples/ranged_unprotected.png}
    \caption[halberdiers unprotected]{\textit{Because Halberdiers are not behind a non-destroyed Wall, \textbf{protection doesn't work}.
...
\begin{itemize}
  \item Players Activate their Units in decreasing order of Unit \pagelink{Initiative}{Intiative}.
  \item When a Unit Activates, place a Faction Cube on it to indicate it has been Activated this Combat Round.
  \item Activated Units may move and attack according to their \pagelink{Unittype}{type}.
...
In Combat against \textbf{Neutral Units}, if you \pagelink{Timelimit}{run out of time} or your Units are defeated, you must \textbf{Retreat}; move any surviving Units back to your Unit Deck and move the Hero that started the Combat back to the Field they last Visited.
There are no other negative consequences to Retreating.\par
\note{9}{
  \textbf{Important exception}: If \textbf{all} of your Units from your Unit Deck were defeated during Neutral Combat, your Main Hero must be moved to a friendly Town or Settlement.
...
You may \textbf{Surrender}\index{Surrender} to other players by paying the other player 10 \includesvg[height=10px]{\svgs/gold.svg} when activating a Unit.
...
\subsection*{\pagetarget{AIrules}{Player vs AI}}
...
The AI Deck consists of cards that are similar in function to Abilities and Artifacts, but change depending on the game's \pagelink{Difficulty}{Difficulty}.
