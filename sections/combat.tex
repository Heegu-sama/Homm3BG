...
\begin{center}
  \transparent{0.3}\includegraphics[width=0.55\linewidth]{\art/wyvern.png}
\end{center}
...
A combat against Neutrals has 3 possible endings: 
\begin{itemize}
	\item If all neutral units are defeated the players Hero is the \textbf{winner}.
	\item If the player runs out of time or loses all combating units, but has still units in his Unit Deck, the Hero performs a \textbf{Retreat}.
	\item If the player loses all combating Units and haven't any Units in his Unit Deck that Hero is \textbf{defeated}.
\end{itemize} 

A combat against another players Hero has 2 possible endings:
\begin{itemize}
	\item If all units on one side are defeated, the combat ends immediately, and the side with any surviving units is the \textbf{winner}, the other side is \textbf{defeated}.
	\item Instead of activating one of your Units, a Hero can decide to \textbf{Surrender} to the other players Hero by paying them 10 \svg{gold} to prevent his army. In this case the combat ends immediately without having a winning or defeated hero.
\end{itemize} 
\note{4}{You cannot surrender when defending a Town.}\par
When Combat ends, all damage is healed from all surviving Units. Move any players owned Units back to their Unit Deck and discard any leftover enemy Neutral Units. Depending on the type and result of the combat, players must resolve the following consequences for themselves:

A \textbf{winning Hero} of a combat remains on the map, gets \pagelink{Combatexperience}{Experience}, if it was the main hero and after that he must Visit the Field where the combat took place.\par
A \textbf{Retreating Hero} from a combat with Neutrals has to move back to the last visited Field on the map.\par
A \textbf{defeated Main Hero} has to move to a friendly town or settlement. Was he defeated by another players Hero, he loses 1 Moral and has to pay 5 \svg{gold} to the winning Hero. Furthermore he gives a \pagelink{End}{Faction Cube} to the winning player. Defeating a Main Hero may cause \pagelink{End}{Player Elimination}.\par 
A \textbf{defeated Secondary Hero} (even instantly defeated without combat) is removed from the map. If defeated by another players hero they gets also a Faction Cube.\par
A \textbf{Surrendering Hero} doesn’t count as defeated. Move your surrendered main hero to a town or settlement, or remove your secondary hero from the map as if he had been defeated. The other player neither get Experience nor Faction Cubes.\par
...
\vspace*{\fill}
\hspace{-2em}{\transparent{0.3}\includegraphics[width=\linewidth]{\art/gremlin.png}}
...
When you defend your Town and it has a Citadel, you add the 3 Wall and 1 Gate Obstacles in any order to the Middle Row of the Combat Board after placing your Units. \textbf{Don't use Walls and Gate}, if you defend an enemy town, which you've conquered before, \textbf{even not} if it is a town with a citadel. The Gate Card is \textbf{not an Obstacle to the defending player}. Defending units may move through the gate and may stop on it. 
\par
The Wall and Gate\index{Walls and Gate} cards can be destroyed by any adjacent \svg{unit_ground} or \svg{unit_flying} Unit's attack, even by your own defending units. These Attacks don't count as normal attacks: They are always successful and they don't activate any of the attacking unit's \svg{unit_attack} abilities and you cannot play any cards or throw the attack die to affect the attack's result.
\par
...
\end{center}
The defender also gains the Arrow Tower\index{Arrow Tower} Unit Card which is placed next to the Combat Board. It acts like any Ranged unit, except that it is not affected by anything related to its positioning. For example it never gets ranged penalty against enemy units and vice versa. Only Ranged attacks and spells can destroy the Arrow Tower, but it is instantly destroyed, when all Walls and the Gate are destroyed.
The attacker doesn't need to destroy it to win the Combat.
\par
\note{8}{In a siege against AI, the Arrow Tower follows the priority-rules like a \svgunit{silver} Unit. But if there are more valid targets, it cannot attack the nearest one, so it will attack the one that is closest to perishing (smallest difference between \svg{health_points} and current \svg{damage}).}
\end{multicols}
