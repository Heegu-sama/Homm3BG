...
\pagetarget{Combat}{Combat}\index{Combat} with \textbf{Neutral Units} starts in two ways:
\begin{itemize}
	\item A Hero moves to an \textbf{unvisited} Field with a roman numeral signifying the \hyperlink{Difficulty Table}{type and number} of Neutral Units guarding that Field. 
\begin{expansion}{navalbattles}
	\item A Hero moves to an \textbf{unvisited} \pagelink{Creature Banks}{Creature Bank Location}, which has slightly different rules for combat setup.
\end{expansion}
\end{itemize}
...
\begin{expansion}{conflux}\textbf{\pagetarget{Elemental Damage}{Elemental Damage}} – If Units deal elemental damage, they attack normally, but the resulting damage works similary to \svg{damage} dealt by spells and it ignores the unit's \svg{defense}. Elemental damage doesn't count as damage from spells.\end{expansion}
...
\begin{expansion}{navalbattles}
	As an \textbf{optional rule} you can use the Naval Battle Board for combats at sea locations. At this combat board the two middle spaces are removed, so that Ground \svg{unit_ground} units must navigate around these spaces and Flying \svg{unit_flying} Units can move over them, but still cannot end their movement there. 
	
	ASSET HERE
\end{expansion}
\vspace{1em}
\begin{expansion}{battlefield}
	As an \textbf{optional rule} you can use the Battlefield Board in combats between players in regular games. It is possible, but not recommended to use it also in neutral combats. Read the rule changes about this combat in the official battlefield rulebook.
\end{expansion}
