...
Check the \pagelink{AI Units}{\textbf{AI Units}} section for detailed instructions.
\end{itemize}
...
A combat against Neutrals has three possible outcomes:
\begin{itemize}
...
A combat against another player's Hero has two possible outcomes:
\begin{itemize}
...
\par
\vspace{1em}
\note{9}{In a siege against AI, the Arrow Tower follows the priority-rules like a \svgunit{silver} Unit. But if there are many valid targets, it cannot attack the nearest one, so it will attack the one that is closest to perishing (the smallest difference between \svg{health_points-note} and current \svg{damage-table}).}
