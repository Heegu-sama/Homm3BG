...
\subsection*{Morale Cards in regular games}\index{Morale Cards}

You can use Morale Cards \textbf{instead} of Tokens. During setup remove all Morale Cards with the battlefield symbol. Then shuffle the positive and negative Morale Card Decks separately and place them face down near the round tracker.

MORALE CARD ASSETS HERE

Every time you gain Positive Morale, discard a negative Morale Card (if you have any); otherwise draw 1 positive Morale Card. You may have \textbf{2 positive Morale Cards at a time}. Don't draw a third card. Every time you lose Morale, discard a positive Morale Card (if you have any), otherwise  draw  1 negative Morale Card. You may have an \textbf{unlimited number of negative Morale Cards}.\par
Place newly gained Morale Cards face up near your Hero Board. Positive Morale Cards may be used at any moment you wish. Negative Morale Cards must be used the next time when the situation indicated by the card occurs. Put used cards at the bottom of their decks.\par 
If you must draw a moral card but the respective Morale Deck is empty, reshuffle their discard pile to form a new deck.

\note{5}{Keep in mind that in Adventure- and Skirmish-mode the rules for morale cards differs a bit, read in official rulebooks.
}
