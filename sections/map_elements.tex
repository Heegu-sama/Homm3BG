...
\pl{
  \addsection{Elementy Mapy}{\skills/logistics.png}
}
...
\pl{
  \subsection*{Anatomia Kafelka Mapy\index{Kafelek Mapy}}
}
\columnbreak
\includegraphics[width=\linewidth]{\images/maptiles.png}
\vfill
\includegraphics[width=\linewidth]{\images/fields.png}
\end{multicols*}
...
\pl{
  \subsection*{\hypertarget{Typy Lokacji}{Typy Lokacji}}
}
\begin{itemize}
  \en{
...
\medskip
\note{6}{
  \en{When you Visit a Visitable field, \textbf{you must} place a black cube on that Field even if you cannot or choose not to use that Field's effects.}
  \pl{}
}
...
\en{\textit{The Combat lasted for only one Round, so Alice would not have been able to cast both Lightning Bolt and Haste, since players are limited to playing only one Spell Card per Combat Round.}\par}
\pl{}
