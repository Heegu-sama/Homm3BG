...
\end{multicols*}
\begin{multicols}{2}
\subsection*{\pagetarget{Discover Location Tokens}{Placing and Discovering Location Tokens}}
...
\begin{expansion}[before=\vspace*{0pt}]{cove}
  ...
  Discovering \pagelink{Whirlpool}{Whirlpools} follows the same rules as Monoliths.
  \bigskip

  \begin{tikzpicture}
    \draw (-2.5, 0) node[inner sep=0] {\includegraphics[width=0.4\linewidth]{\map_locations/whirlpool-token.png}};
    \draw (-2.5, -1) node {\Huge\textbf{\textcolor{white}{-1}}};
    ...
    \draw (2.5, 0) node[inner sep=0] {\includegraphics[width=0.4\linewidth]{\map_locations/whirlpool-token.png}};
    \draw (2.5, -1) node {\Huge\textbf{\textcolor{white}{$\boldsymbol{+}$1}}};
  \end{tikzpicture}
  \begin{center}
...
\begin{expansion}[before=\vspace*{0pt}]{navalbattles}
  ...
  If you do, each time you discover a Far \mbox{(II--III)} or Near \mbox{(IV--V)} Map Tile, you may decide to draw a Creature Bank Token corresponding to the discovered Map Tile, and -- contrary to the normal rules -- place it \textbf{only on its Blocked Field}.
  \bigskip
  \begin{center}
