...
\begin{center}
  \begin{scriptsize}
    ...
    \begin{tikzpicture}
      ...
      \node[rotate=-60] (gate) at (1.6, -0.53) {\includegraphics[width=0.33\textwidth]{\images/subter-gate-outline.png}};
...
\subsection*{Discovering Tiles with Location Tokens}
Location Tokens are added by several expansions to the game. They are placed on specific Fields of the map, when a hero discovers the associated Map Tile and they overwrite the original location they are placed on. 
The following locations are released and they follow slightly different rules when discovered.
\begin{itemize}
\begin{expansion}{cove}
  	\item The Position of \pagelink{All Map Locations}{\textbf{Whirlpool Tokens}} are determined by the chosen Scenario Layout. If you discover a Tile with one of these tokens on it, discover the Tile normally and then place the Token on that Field indicated by the Scenario's map layout. Tokens cannot be placed on blocked Fields, other Location Tokens, or Fields containing Locations required to meet any of the Scenario's victory conditions. Keep these rules in mind when discovering and rotating the Tile.
\end{expansion}
\begin{expansion}{conflux}
  	\item Discovering \pagelink{All Map Locations}{\textbf{Monolith Tokens}} follows the same rules like Whirlpools.
\end{expansion}
\begin{expansion}{stronghold}
  	\item \pagelink{All Map Locations}{\textbf{Subterranean Gate Tokens}} are also determined by a scenario's Map Layout and they connect both a Surface Map Tile and an adjacent Subterranean Map Tile. After discovering one of the corresponding Map Tiles you may choose the position of the Subterranean Gate Token as long as it connects the two indicated Map Tiles and doesn't occupy any blocked fields, other Location Tokens or Locations required for victory conditions. 
\end{expansion}
\begin{expansion}{naval battles}
  	\item As an \textbf{optional rule} you can decide to play with \pagelink{All Map Locations}{\textbf{Creature Banks}}. If you do, each time a player discovers a Far (II-III) or Near Map Tile (IV-V), he may decide to place a Creature Bank Token corresponding to the discovered Map Tile on one of their blocked fields.
\end{expansion}
\end{itemize}
\vspace*{\fill}
