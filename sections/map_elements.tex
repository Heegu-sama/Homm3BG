...
They are placed on specific Fields of the Map when a Hero discovers the associated Map Tile and overwrite the original Location they are placed on.
Tokens cannot be placed on other Location Tokens or Fields containing Locations required to meet any of the Scenario's victory conditions.

\begin{expansion}[before=\vspace*{0pt}]{conflux}
  \subsection*{Monolith Tokens}
   The position of \pagelink{Monolith}{\textbf{Monolith Tokens}} is determined by the chosen Scenario Layout.
...
\begin{expansion}[before=\vspace*{0pt}]{cove}
  ...
  Discovering \pagelink{Whirlpool}{\textbf{Whirlpool Tokens}} follows the same rules as Monoliths.
  \bigskip
  \begin{center}
...
\begin{expansion}[before=\vspace*{-11mm}]{stronghold}
  ...
  \pagelink{Subterranean Gate}{\textbf{Subterranean Gate Tokens}} are also determined by a Scenario's Map Layout and connect both a Surface Map Tile and an adjacent Subterranean Map Tile.
  After discovering one of the corresponding Map Tiles you may choose the position of the Subterranean Gate Token as long as it connects the two indicated Map Tiles and doesn't occupy any blocked Fields, other Location Tokens or Locations required for victory conditions.
  The Scenario Map layout only shows which two Tiles are connected, not the exact Fields that must be connected.
  \bigskip
  \begin{center}
...
\begin{expansion}[before=\vspace*{0pt}]{navalbattles}
  ...
  As an \textbf{optional rule} you can decide to play with \pagelink{Creature Banks Rules}{\textbf{Creature Banks}}.
  If you do, each time a player discovers a Far \mbox{(II-III)} or Near \mbox{(IV-V)} Map Tile, they may decide to place a Creature Bank Token corresponding to the discovered Map Tile on its blocked Field.
  \bigskip
  \begin{center}
...
\begin{expansion}{stronghold}
  ...
  They work similarly to regular Map Tiles, except that \textbf{you cannot move between a Surface and a Subterranean Tile without using a Subterranean Gate in between} or a Town Portal Spell.
  No other movement effects from Cards can allow you to move from one to the other.
  You also may not discover a Subterranean Map Tile while standing on a Surface Map Tile and vice versa.\par
...
\begin{center}
  ...
  \begin{tikzpicture}
    ...
    \draw (-2.2, 0.1) node {\textbf{\textit{\textcolor{darkcandyapplered}{Elemental Fire Tile}}}};
    \draw (-2.2, -4.5) node {\textbf{\textit{\textcolor{darkcandyapplered}{Elemental Air Tile}}}};
    \draw (2.2, 0.1) node {\textbf{\textit{\textcolor{darkcandyapplered}{Elemental Water Tile}}}};
    \draw (2.2, -4.5) node {\textbf{\textit{\textcolor{darkcandyapplered}{Elemental Earth Tile}}}};
  \end{tikzpicture}
\end{center}
