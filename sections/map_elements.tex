...
Location Tokens are added by several expansions to the game and are placed on specific Fields of the map, as shown in the Scenario's map layout.
\textbf{Unless otherwise stated}, the following rules apply to all Location Tokens:
\begin{itemize}
  ...
  \item Tokens cannot be placed on blocked Fields, other Tokens, or Fields containing Locations required to meet any of the Scenario's victory conditions.
    Keep these rules in mind when discovering and rotating a Tile with a Token on it.
\end{itemize}
\vspace*{\fill}
