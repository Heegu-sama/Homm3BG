...
The \textbf{Roman numeral} on each Tile describes the overall \textbf{difficulty of Neutral Units} on that Tile.
...
\begin{itemize}
  ...
  \item \textbf{Revisitable} – You can visit this Field multiple times.
    ...
    If you are already on this Field, you may pay 1 MP to visit this Field again.
\end{itemize}
\vspace*{\fill}
\columnbreak
...
\subsection*{Example ``Empowering Spells and flagging a guarded Mine''}
\pagetarget{example flagging mine}{}
...
\includegraphics[width=\linewidth]{\examples/sandro_takes_mine.png}

\textit{The Mine turns out to be guarded by Griffins with 4~\svg{health_points}.
  Alice's current hand consists of a Power card, a Magic Arrow, Sorcery, and a Town Portal.
  During the Combat, Alice decides to cast an offensive Spell, as she already holds a Magic Arrow.
  She first plays Sorcery for its \svg{empower} effect, which also draws her a card: a Lightning Bolt.
  She casts the Lightning Bolt instead, empowering it with the already played Sorcery and with Magic Arrow's alternative (bottom) effect.
  The Lightning Bolt deals 4~\svg{damage}, killing the Griffins, so Alice wins the Combat.
  The Combat lasted only one Round.
  Alice could not have cast both Magic Arrow and the drawn Lightning Bolt, because players may play only one Spell card per Combat Round.
  For winning the Combat, Sandro gains his second \svg{experience}, reaching Level II, so Alice Searches~(2) the Ability Deck before visiting the Field.}\par

\textit{Alice now flags the Mine by placing one of her Faction Cubes on it.
  Flagging this Mine increases her Building Materials \svg{building_materials} production by 2, and she immediately gains the Mine's production value of 2~\svg{building_materials} because she was the first player to flag it.
  Afterwards, Alice wants to go back to defend a previously flagged Settlement by casting the Town Portal still in her hand.
  Her Hero is Level II, so she can empower the Spell with the Power card's Expert Effect \svg{expert}, which grants her an additional Movement Point after casting it.
}
...
{
  \includegraphics[width=0.6\linewidth]{\examples/sandro_empowering_lightning_bolt.png}\hfill
  {\transparent{0.2}\includegraphics[width=0.3\linewidth]{\art/resurrection.png}}
}
...
\begin{expansion}{stronghold}
  ...
  They work similarly to regular Map Tiles, except that \textbf{you cannot move between a Surface and a Subterranean Tile without using a \pagelink{Subterranean Gate}{Subterranean Gate} in between} or a \wikilink{spells/town_portal}{Town Portal} Spell.
...
\AddToHookNext{shipout/background}{%
  \iftoggle{noartbackground}{}{
    \begin{tikzpicture}[remember picture,overlay]
      \node(bg)[anchor=south east, opacity=0.15, xshift=2em, yshift=-5em] at (current page.south east) {
...
\pagetarget{Discover Location Tokens}{\subheader[Placing and Discovering\\Location Tokens]{Placing and Discovering Location Tokens}}
\begin{multicols}{2}
...
\begin{expansion}[before=\vspace*{5pt}]{navalbattles}
  \pagetarget{Creature Bank Tokens}{\subsection*{Creature Bank Tokens}}\index{Creature Banks}
  \setlength\intextsep{3pt}
  \setlength\columnsep{0.5em}
  \begin{wrapfigure}{l}{0.38\linewidth}
