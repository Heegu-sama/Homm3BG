...
\textit{The Mine turns out to be guarded by Griffins, which have 4 HP \svg{health_points}.
Alice's current hand consists of a Power card, a Magic Arrow, Sorcery, and a Town Portal.
During the Combat, Alice plays Sorcery first hoping to draw an additional helpful card: She draws a Lightning Bolt, which she now decides to cast, and Empowers \svg{empower} it with the already played Sorcery's effect and Magic Arrows's alternative (bottom) effect, which makes the Lightning Bolt deal 4 damage \svg{damage}, killing the Griffins and winning the Combat.}

\textit{The Combat lasted for only one Round, so Alice would not have been able to cast both Magic Arrow and the drawn Lightning Bolt, since players are limited to playing only one Spell card per Combat Round.}\par
