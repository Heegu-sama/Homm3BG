\addsection[2]{Skirmish and \break{}Adventure Mode}{\spells/berserk.png}

\begin{expansion}{battlefield}
\begin{multicols}{2}
\subsection*{Overview}
    The Skirmish and Adventure Mode are similarly structured 1v1 game modes whose central idea is to wage a tactical combat on the large Battlefield Board.\par
    \smallskip
    The two modes differ in the way players build their Unit and Player Decks:\par
    \smallskip
    In \textbf{Skirmish Mode} the players simply get pre-constructed Decks, on whose composition they have only a little influence.
    With these Decks the players jump directly into Combat.\par
    \smallskip
    In \textbf{Adventure Mode}, a card-driven Adventure Phase is played before the Combat, which replaces and simulates the discovery of the Map from regular games.
    In this phase, players use Adventure Cards to gather Resources, expand their Town, and develop their Heroes, thus deciding for themselves which Units to Recruit and which types of Cards to include in their Deck, rather than limited on pre-constructed sets.\par
    \smallskip
\subsection*{Victory Condition}
    To achieve a victory, one of the players must defeat all enemy Units in the final Combat.\par
    \smallskip
\subsection*{Exception for Morale Cards}
    Both modes use \textbf{\pagelink{Morale Cards}{Morale Cards}} instead of Tokens with one exception, when resolving a \svg{morale_positive} effect:
...
\subsection*{Dealing with rule ambiguities}
    \textbf{\textit{\textcolor{darkcandyapplered}{Important Note:}}}
    Both game modes were developed only with the \textbf{Core Game in mind}.
    ...
    \begin{itemize}
        ...
        \item Which Cards should be removed in Skirmish (cause there only Combat effects are useful)?
        \item How to handle Cards with Permanent Effects?
        \item How to handle Cards with Remove Effects?
        \item How to handle Cards with both a Combat and an Adventure Effect?
    \end{itemize}
    }\par
    \smallskip
    \begin{center}
        {\transparent{0.4}{\includegraphics[width=0.5\linewidth]{\art/cannon.png}}}
    \end{center}\par
    \vspace*{\fill}
    \end{multicols}
\end{expansion}
...
\begin{expansion}{battlefield}
    ...
    After Setup, Both players roll 2 \svg{resource_die}. The one with the highest results is the first player.
    Use the Initiative Token to track the current first player.
    Set your starting Deck aside for now.
    You only need it in the Combat Phase.
    \end{multicols*}
\begin{center}
...
\begin{expansion}{battlefield}
    ...
    The Skirmish Mode has no adventure phase.
    You skip hat phase and you jump straight into Combat.\par
    \vspace*{1em}
    ...
    Then follow the rules for \pagelink{Battlefield Combat}{Combat on the Battlefield Board} with one additional rule: At the end of each Combat Round \textbf{regain one used \svg{expert}} up to Your Hero's Limit and \textbf{draw 2 Cards from your Deck}.\par
...
\begin{expansion}{battlefield}
    ...
    The Adventure Mode is divided into two Phases: The \textbf{Adventure Phase} and the \textbf{Combat Phase}. The latter works identically as in the Skirmish Mode.\par
    ...
    Follow these steps in the following order during each Round.\par
        \begin{enumerate}
            ...
            (\textbf{Note:} Each player may resolve \svg{movement} effects to discard any number of drawn Adventure Cards and redraw that many Cards, hopefully to get a better selection.)\par
        ...
        \end{enumerate}\par
        \smallskip
    In the last Round the players don't draw Adventure Cards, but they can still resolve a saved Adventure Card. After the last round, the Adventure Phase ends and the Combat Phase starts.
    ...
    They remove all \svg{paralysis} from recruited Units, take Miniatures of all of them and start the Combat on the Battlefield Board following \pagelink{Battlefield Combat}{the general rules for that}.
...
\begin{expansion}{battlefield}
    ...
    \begin{multicols}{2}
        ...
        \end{center}
    \end{multicols}
    To resolve an \textbf{Adventure-Event Card} just resolve the printed effect. The effect of the Event Card "Obelisk" grows stronger with every own Obelisk Card played before.\par
    \smallskip
    %clarification by heegu? https://boardgamegeek.com/thread/3275803/article/44055341#44055341
    ...
    \begin{enumerate}
        ...
        Select up to 2 of your Units and roll 1 Attack die for each.
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item Use the \pagelink{Morale Cards}{Morale Cards} instead of Morale Tokens in Adventure Mode with one exception:
        When resolving a \svg{morale_positive} effect instead of randomly draw \textbf{one} Postive Morale Card you \textbf{Search(2)} the Positive Morale Deck instead.
        All other rules remain untouched.
    \end{itemize}
    \end{multicols}
\end{expansion}