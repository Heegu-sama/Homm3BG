...
\pagetarget{AIrules}{\addsection{AI Rules}{\spells/fortune.png}}

\begin{multicols*}{2}

\subheader{AI Hero Turn}
AI Heroes are used in the Campaigns.
They start in their Town, take turns after the player, and have 3 MP, always spending them to perform the following Actions in descending priority:
\begin{itemize}
...
Their units are static and defined by the Scenario's setup or other rules: find the indicated neutral and faction unit cards when you start the scenario.\par
...
If Skill cards are included, search for and set aside the card related to it (usually, an Ability).
...
AI Skill cards cannot be replaced, so if setup assigns the AI Hero a card that your Hero has and you don't have another copy, remove the needed card from your Hero's deck and \textbf{Search (3)} the respective card's deck to compensate your Hero for the loss.

\subsection*{AI Magic \svg{magic} cards}
...
If the AI Hero is assigned a Spell card that is unavailable because it's your Hero's Deck, and you don't have another copy, substitute it with a Magic Arrow card.
...
If your Hero is assigned a Spell, Artifact, or Ability card that is unavailable, \textbf{Search (3)} the respective card's deck to get another card in its stead.
...
\subsection*{Casting complex spells}
...
If there is more than one valid group, attack the one difference that is between the closest to perishing (has the smallest difference between \svg{health_points} and current \svg{damage}).
