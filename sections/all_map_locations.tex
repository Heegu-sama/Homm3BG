...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.47\textwidth}
  	\vspace{0pt}
  	\centering
  	\textbf{Pandora's Box (Variant)}\par
  	\framedimage[\linewidth]{\map_locations/pandoras_boxes.jpg}
  	\caption{\small Category: \textbf{Visitable}\\
  		\begin{expansion}{stretchgoals2}	
  			\textbf{Game Variant for more randomness:}
  			Choose one: Resolve the printed effect normally OR draw a Pandora's Box Card and resolve its effect. 
  		\end{expansion}
  	}
  \end{minipage}\hfill
\end{figure}
...
\subsection*{\pagetarget{Location Token}{Location Token and Creature Banks}}
...
\begin{figure}[H]
	\begin{minipage}[t]{0.47\textwidth}
		...
		\caption{\small Category: \textbf{Revisitable}\\
			...
			After each such travel, lose 1 Unit from your Unit Deck. You can Recruit it again later.} % What does losing mean: complete unit card? Or reduce pack to few?
	\end{minipage}\hfill
	\begin{minipage}[t]{0.47\textwidth}
		\vspace{0pt}
		\centering
		...
		\caption{\small Category: \textbf{Visitable}\\
			When you enter a Creature Bank (see \pagelink{Creature Banks}{List} below), start a \pagelink{Creature Bank Rules}{Creature Bank Combat} against the specified units. If you win, receive the reward and the bonus (depending on defeated stacked units). Creature banks count as \textbf{land fields} (even if designed as sea locations), and should not be confused with Other Fields of the same name.
		}
	\end{minipage}
\end{figure}
