...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.47\textwidth}
    ...
    \caption{\small Category: \textbf{Visitable}\\
      For every Dwelling you have, draw from the corresponding Neutral Unit Decks until you find ``Elementals'' Cards.
      You can recruit one of those Units if you  \svg{pay_v2} its recruitment cost.
      Shuflle the remaining Units back into ther respective Decks.
    }
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  \begin{minipage}[t]{0.47\textwidth}
    ...
    \caption{\small Category: \textbf{Visitable}\\
      When preparing the Neutral Army guarding this location, instead of drawing 1 \svgunit{azure} Unit, find 2 \svgunit{golden} Cyclops and add them to the Neutral Army (look for them first in the \svgunit{golden} discard pile, and then in the \svgunit{golden} Neutral Uni Deck).
