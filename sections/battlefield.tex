...
\begin{expansion}{battlefield}
  ...
  To achieve victory and Skirmish or Adventure, the player must defeat all enemy units in the final Combat.
...
\begin{expansion}[before=\vspace*{-11mm}]{battlefield}
  ...
  The starting player receives the \pagetarget{Initiative Token}{\textbf{Initiative Token}}, which will be used it to break any Initiative ties of opposing units.
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{\images/initiative-bf.png}
    \caption{\imagecaption{Initiative Token}}
  \end{figure}
  % If two or more opposing units have the same Initiative value, the player with the Token activates \textbf{one of their units first}.
  % Then, the players alternate in activating all of their units of that same Initiative value one by one.
  % Once all units with that Initiative value are activated, pass the Initiative Token to the other player, who keeps it until an initiative tie of opposing units happens again.

  \subsection*{\pagetarget{BF Obstacles}{Obstacle Tokens}}
  You can place three token types on the Battlefield Board: Effect, Obstacle and Wall/Gate.
  ...
  Starting with the player with the Initiative Token, both players take turns choosing and placing one Obstacle Tokens on the Battlefield Board one-by-one until \textbf{all Obstacle Tokens (no Effects yet) are placed}.
  ...
  {\footnotesize\imagecaption{Obstacles and Miniatures placed correctly are marked with the green border, and incorrectly -- with the red one.}}\par
  \vspace{1em}
    ...
    The 5-unit limit does not apply in Battlefield modes, and players may place all 7 Faction units.
...
\begin{expansion}[before=\vspace*{-11mm}]{battlefield}
  ...
  \begin{itemize}
    ...
    \item At the end of each Combat Round, \textbf{regain one used \svg{expert}} up to your Hero's Limit, and \textbf{draw 2 cards from your Deck}.
  \end{itemize}
...
\begin{expansion}[before=\vspace*{-11mm}]{battlefield}
  ...
  \begin{multicols*}{2}
    Although Battlefield is designed for \pagelink{Battlefield Modes}{Adventure and Skirmish modes}, it is possible to use the Battlefield Board in other game modes, albeit with slight rule modifications.

    \subsection*{Changes to Combat Rules}

    \begin{itemize}
      \item The attacker receives the Initiative Token.
      \item In player vs. player Combat, you may place up to 7 units.
        In Neutral Combat, the 5-unit limit still applies.
      \item You regain no \svg{expert} nor draw any cards from your Deck at the end of Combat Rounds.
    \end{itemize}

    \subsection*{Siege with Walls and Gate}
    During a Siege against a Town with a Citadel, only Walls and Gate Obstacles are placed.
    ...
    If you decide to use the Battlefield Board in Combats with Neutral Units, ignore the Round Limit: at the end of each Combat Round you may freely decide to Retreat or to gain an additional Combat Round with no need to spend a MP \svg{movement}.
