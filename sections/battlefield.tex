...
\begin{expansion}[before=\vspace*{-11mm}]{battlefield}
  ...
  \begin{multicols*}{2}
  Battlefield is compatible with regular games and \pagelink{Battlefield Modes}{Adventure with Skirmish}, though the rules vary slightly.
  For the regular games, the attacking player gains the Initiative Token at the start of the Combat.
  ...
  \begin{itemize}
    \item \textbf{Place obstacles} -- Starting with the player with the Initiative Token, both players take turns choosing and placing one Obstacle Token on the Battlefield Board until \textbf{all Obstacle Tokens (no Effects yet) are placed}.
    No Obstacle can be adjacent to another Obstacle or to any player's deployment zone.

    {\centering
      \begin{tikzpicture}
        \draw (0, 0) node {\framedimage[0.7\linewidth]{\examples/bf-deploy.png}};
    ...
    \item \textbf{Place units} -- The player with the Initiative Token chooses a deployment zone and places one of their units in it, then players take turns placing their units \textbf{one by one} each in their zone.
    \begin{itemize}
      \item Skirmish, Adventure -- Place all your available units.
      \item Regular games, Neutral Combat -- Place up to 5 units.
      \item Regular games, PvP -- The players can decide to allow placing up to 5 units or \textbf{up to 7 units}.
    \end{itemize}
...
\begin{expansion}[before=\vspace*{-11mm}]{battlefield}
  \begin{multicols*}{2}
  \begin{itemize}
    \item \textbf{Siege with Walls and Gate} (this normally happens only in regular games, not in \pagelinkwithcomma{Battlefield Modes}{Adventure/Skirmish}) -- During a Siege against a Town with a Citadel, only Walls and Gate Obstacles are placed.
      Place no other Obstacles.

    \begin{tikzpicture}
      \draw (0, 0) node {\framedimage[1.05\linewidth]{\examples/bf-deploy-wall.png}};
      \draw (2, 0) node[rotate=90] {\textbf{\fontfamily{ptm}\selectfont\textcolor{white}{DEPLOYMENT ZONE}}};
    \end{tikzpicture}
  ...
  \begin{itemize}
    ...
    \begin{minipage}{\linewidth}
      ...
      \end{wrapfigure}
      \item \textbf{\pagetarget{Initiative Token}{Initiative Token}} -- After the setup, use it to break any Initiative ties of opposing units.
    ...
    \item \textbf{In \pagelink{Adventure and Skirmish Modes}{Adventure and Skirmish games} only:}
      At the end of each Combat Round, \textbf{regain one used \svg{expert}} up to your Hero's Limit and \textbf{draw 2 cards from your Deck}.
  \end{itemize}
    \vspace*{\fill}
  \end{multicols*}
\end{expansion}
