...
When playing a Spell card for its primary effect (``casting the Spell''), choose how much Power \svg{empower} (3) you'll spend from the \textbf{Power Pool} for it, and apply the chosen effect (4).
The topmost effect usually requires spending 0~\svg{empower}.
...
During Combat, each player may play \textbf{only one Spell card per Combat Round} for its primary effect, though some effects like \svg{expert} \wikilink{statistics/knowledge}{Knowledge} can increase this number.
...
Some Spell cards with complex effects use keywords or require special components, see \pagelink{spell keywords}{Card Effect Keywords}.
\columnbreak

\subsection*{Empowering Spells}
Some effects can increase your \textbf{Power Pool} \svg{empower}: for example, the \wikilink{statistics/power}{Power} Statistic card, or the alternative bottom (5) +1~\svg{empower} effect of Spell cards.
Keep played \svg{instant} cards which increased your \svg{empower} on the table, don't discard them yet.
When you do anything which doesn't add any more \svg{empower} to the Pool, all \svg{empower} in the Pool disappears; that includes the case of casting a Spell, even if you used not all available \svg{empower} from the Pool.
At that moment, discard all the \svg{instant} cards you played which added \svg{empower}.
Count how much \svg{empower} you spent on the Spell, and choose which of these cards participated in it; one of the cards may be used partially if the sum is too high.
These cards are considered played ``together'' with the Spell, this is important for certain effects such as \svg{expert} \wikilink{abilities/mysticism}{Mysticism}.

See examples of Empowering on \hyperlink{example flagging mine}{page \pageref{example flagging mine}}.

\textbf{Note:} \pagelink{Elemental Map Tiles}{Elemental Map Tiles} increase Spells' \svg{empower} without the Pool.
