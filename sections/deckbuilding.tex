...
All Statistic\index{Statistic cards} and Ability cards\index{Ability cards} have a \textbf{Basic Effect}\index{Basic Effect} and a stronger \textbf{Expert} \svg{expert} \textbf{Effect}\index{Expert Effect}, which is shown below the Basic Effect.
...
Using the topmost, basic version usually has no additional cost, but by paying the indicated cost \encircleinline{3} you can use a stronger effect \encircleinline{4}.
You pay by playing other cards for their \textbf{Empower}\index{Empowering Spells} \svg{empower} effect (e.g. the \wikilink{statistics/power}{Power} Statistic card) together with the Spell.
All the \svg{empower} played this way is consumed.
...
Some Details about Spells:\par
\begin{itemize}
    \item You may use a cheaper effect than the \svg{empower} you paid for.
    \item If you gain \svg{empower} effects but no Spell is played at all, that \svg{empower} is lost.
    You can't save it for later use.
    \item Playing the cards together means you can resolve their effects in any order, even resolve cards with Empower effects before decide which Spell to cast.
    \item During Combat, each player may play \textbf{only one Spell card per Combat Round} for its primary effect, though some effects such as \svg{expert} \wikilink{statistics/knowledge}{Knowledge} can increase this number.
    \item When you use an effect which lets you cast a Spell from somewhere other than your hand (such as directly from the Spell Deck), place that Spell into your Discard Pile after casting~it.
...
\begin{center}
  \vspace*{\fill}
  {\transparent{0.2}\includegraphics[width=0.75\linewidth]{\art/air_magic.png}}
  \vspace*{\fill}
\end{center}
