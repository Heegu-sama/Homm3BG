...
All Ability\index{Ability Card} and Statistic cards\index{Statistic Cards} have a \textbf{Basic Effect}\index{Basic Effect} and a stronger \textbf{Expert} \svg{expert} \textbf{Effect}\index{Expert Effect}, which is shown below the Basic Effect.
...
\begin{expansion}[before=\vspace*{-11mm}]{navalbattles}
  ...
  These cards are more powerful versions of the normal Ability cards.
  They have only one effect which is identical to the normal Ability's Expert Effect, but does not require using your \svg{expert}.

  \medskip
  These cards can be gained by spending an \pagelink{Empowered Ability Token}{Empowered Ability Token}.
...
\begin{scriptsize}
  \begin{tikzpicture}
    \draw (-2, 0) node[inner sep=0] {\makebox[0.47\linewidth][c]{\includegraphics[width=0.47\linewidth]{\cards/empowered-ability.png}}};
    \draw (2.4, 0) node[inner sep=0] {\makebox[0.47\linewidth][c]{\includegraphics[width=0.47\linewidth]{\cards/empowered-knowledge.png}}};
    \draw (-1.2, 1.9) node {\encircle{\phantom{.}1\phantom{.}}};
    \draw (3.2, 1.9) node {\encircle{\phantom{.}1\phantom{.}}};
    \draw (-2.7, -1.4) node {\encircle{\phantom{.}2\phantom{.}}};
    \draw (1.7, -1.4) node {\encircle{\phantom{.}2\phantom{.}}};
    \draw (-1, -2.1) node {\encircle{\phantom{.}3\phantom{.}}};
  \end{tikzpicture}
\end{scriptsize}
\vspace*{-2em}
\begin{multicols*}{2}
  ...
  \begin{itemize}
    \item[\textbf{3.}] Necropolis-specific Card
  \end{itemize}
\end{multicols*}
...
\begin{expansion}{inferno}
  ...
  They have only one effect which is identical to the normal Statistic's Expert Effect, but does not require using your \svg{expert}.

  \medskip
  These cards can be gained only via certain game effects, such as Visiting a \pagelink{Star Axis}{Star Axis}.
\end{expansion}
...

{
	\medskip
	\centering
	\includegraphics[width=0.5\linewidth]{\cards/test_spellscroll.png}\\
	\medskip
	\footnotesize
	\begin{center}
...
When you gain a Spell Scroll place it near your Hero Board, take the 2 top cards from the Spell deck and place them face down on the Spell Scroll.
You can look at these spells at any time and they don't count for your hand size limit.
You can only have 2 Spell Scrolls at a time.
If you gain a third one, draw the new spells normally and then decide whether you discard the newly gained Spell Scroll or replace one you already have (including all spells on it).
...
\begin{itemize}
	...
	\item You can only use their lowest \svg{empower} effect.
