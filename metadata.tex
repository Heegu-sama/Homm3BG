...
\newcommand\encirclebase[3]{%
  \ifdim\dimexpr#2\relax<1.6em\relax
    \def\shadowshift{2pt}
    \edef\shadowscale{2.2\dimexpr#2\relax}
  \else
    \def\shadowshift{2pt}
    \edef\shadowscale{1.5\dimexpr#2\relax}
  \fi
  \begin{footnotesize}%
  \tikz[baseline=(X.base)]{
    \node (X) [draw=white, shape=circle, fill=#1, minimum size=#2, align=center, inner sep=0, text=white] {\strut \textbf{#3}};
    \begin{scope}[on background layer]
      \node [inner sep=0pt, opacity=0.7] at ([xshift=\shadowshift, yshift=-\shadowshift]X.center) {\includegraphics[width=\shadowscale]{\layout/circle_drop_shadow.png}};
    \end{scope}
  }
  \end{footnotesize}%
}
