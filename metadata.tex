...
\newcommand{\hommtable}[2]{
  \begin{tikzpicture}
    ...
    \begin{pgfonlayer}{background}
      \begin{scope}[blend mode=multiply]
        \draw [shade, fill=black, opacity=0.15, blur shadow={shadow opacity=15}] ([xshift=0.1pt, yshift=-0.1pt] current bounding box.north west) rectangle ([xshift=-0.1pt, yshift=0.1pt] current bounding box.south east);
      \end{scope}
    \end{pgfonlayer}
  \end{tikzpicture}
}
