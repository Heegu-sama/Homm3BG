...
% LaTeX3 clauses for the purposes of glyphs rendering
\ExplSyntaxOn
\clist_new:N \g_unit_list
\clist_set:Nn \g_unit_list { bronze, silver, golden, azure }

% List of glyphs that don't look good when they're below the text line
\clist_new:N \g_nodrop_base_list
\clist_set:Nn \g_nodrop_base_list { movement, expert, reinforce, level1, level4, level6, unit_flying, arrow_right_gray, arrow_right }

\clist_new:N \g_nodrop_list
\clist_set_eq:NN \g_nodrop_list \g_nodrop_base_list

% Add the -yellow variants to the list automatically
\clist_map_inline:Nn \g_nodrop_base_list {
  \clist_put_right:Nn \g_nodrop_list { #1-yellow }
}

% An "if" defined with latex3 to check if an arg passed to the \svg command is a unit
\NewDocumentCommand{\ifunit}{mmm}{
  \clist_if_in:NnTF \g_unit_list {#1} {#2} {#3}
}

% Some glyphs don't look good when they're below the text line - their list is defined above
...
% Argument 1 is the height in pixels
...
\newcommand{\svg}[2][10]{%
  \iftoggle{noartbackground}{%
    ...
    \IfEndWith{#2}{-yellow}{%
      ...
      % In the economy printable build we append "-mono" to unit glyphs - those are separate glyphs with letters, because in this mode they can't be differentiated by colors
  ...
  \ifnodrop{#2}{%
    ...
    % In all other cases, drop the glyph by -0.1\baselineskip if it's a unit, -0.15\baselineskip otherwise
