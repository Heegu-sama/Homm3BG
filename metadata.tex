...
\NewDocumentEnvironment{expansion}{ O{} m +b }{
  ...
  \iftoggle{textright2left}{
    \tl_set:No \l_expoverlay_tl {
      ...
      ([xshift=-5pt-\linewidth, yshift=-3pt]frame.south~east);
    }
  }{
    \tl_set:No \l_expoverlay_tl {
      \fill[
...
% A toggle for switching off AccSupp, which breaks SVGs in tabular environments
\newif\ifaccsupp
\accsupptrue
...
\newcommand{\svg}[2][10]{%
  ...
  % Only use AccSupp if it wasn't manually switched off for enviroments where spacing is critical
  \ifaccsupp
    \BeginAccSupp{ActualText={<#2>},space}\EndAccSupp{}%
  \fi
  \ifnodrop{#2}{%
...
\newcommand\encirclebase[4]{%
  \begin{footnotesize}%
  \tikz[baseline=(X.base)]
  \node (X) [draw=white, shape=circle, minimum size=#3, align=center, inner sep=0, fill=#1, text=white, #2] {\strut \textbf{#4}};%
  \end{footnotesize}%
}
\newcommand\encircle[1]{
  \encirclebase{goblin}{blur shadow={shadow blur steps=5}}{1.6em}{#1}
}
\newcommand\encirclered[1]{%
  \encirclebase{darkcandyapplered}{blur shadow={shadow blur steps=5}}{1.6em}{#1}
}
\newcommand\encirclelegend[2]{%
  \encirclebase{#1}{blur shadow={shadow blur steps=3, shadow xshift=0.5pt, shadow yshift=-0.5pt, shadow blur radius=1pt}}{1.6em}{#2}
}
\newcommand\encirclesmall[1]{
  \encirclebase{goblin}{blur shadow={shadow blur steps=5}}{0.9em}{#1}
}
