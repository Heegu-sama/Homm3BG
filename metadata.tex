...
% 1) [Optional] Drop shadow opacity percentage, defaults to 15. Set to 0 for no shadow.
% 2) Height specified in verses (lines of text)
% 3) Table contents like title and tabularx environment
\newcommand{\hommtable}[3][15]{
  \begin{tikzpicture}
    ...
    \begin{pgfonlayer}{background}
      \draw [shade, blur shadow={shadow opacity=#1}] (current bounding box.north west) rectangle (current bounding box.south east);
    \end{pgfonlayer}
  \end{tikzpicture}
}
