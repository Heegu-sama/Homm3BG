...
usage() {
  echo "Usage: $0 [language] [-p|--printable] [-n|--no-bg] [-s|--section SEARCH] [--args ARGUMENTS] [-f]"
  ...
  echo "  -f                 Force latexmk to rebuild the document"
...
if [[ -n "${SECTION_SEARCH}" ]]; then
  ...
  # Target is ambiguous
  if [[ $(echo "$TARGET" | wc -l) -gt 1 && "$SECTION_SEARCH" != *'\|'* ]]; then
...
if [[ "${SECTION_SEARCH}" == "" || "${LANGUAGE}" != "en" || "${FORCE_REBUILD}" == 1 ]]; then
...
time latexmk -pdflua -file-line-error -halt-on-error -interaction=nonstopmode -shell-escape -recorder $ARGS "main_${LANGUAGE}.tex"
