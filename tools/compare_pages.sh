...
help() {
  echo "
    ...
    Optional Arguments:
      ...
      -m, --mapping A:B             Page A on the left will be with page B on the right.
...
for page in $pages; do
  ...
  pdftoppm "main_${language}.pdf" "${tmp_dir}/bb" -f "${mapping[${page}]:-${page}}" -l "${mapping[${page}]:-${page}}" -png &
done
...
for page in $pages; do
  ...
  bb_file=$(get_actual_filename "$tmp_dir" "bb" "${mapping[${page}]:-${page}}")
