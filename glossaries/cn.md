# Chinese (cn)

## Style notes

- Round翻译为轮或者轮次，Turn翻译为回合，不可混淆
- 空格规则：
  - 空白包括空格，~, \n
  - 中文段落中避免出现空白
  - 如果翻译字符串内出现\n，且此行于下一行同属于一个段落，可以使用%\n避免生成空白
  - 如果latex命令（例如\textbf,\textit,\svg,\pagelink, \wikilink）出现在汉字段落中，则去掉命令前后原有的空白。例如，`汉字\svg{icon}汉字`或是`1\svg{gold}`，而不是`汉字 \svg{icon} 汉字`或`1 \svg{gold}`
- 如果wikilink的文字中包括标点符号，视情况将标点符号移动到wikilink文字之外，或直接删除

## Factions

| English | Chinese |
|---|---|
| castle | 城堡 |
| conflux | 元素城 |
| cove | 港口 |
| dungeon | 地下城 |
| fortress | 要塞 |
| inferno | 地狱 |
| necropolis | 墓园 |
| rampart | 壁垒 |
| stronghold | 据点 |
| tower | 塔楼 |

## Units

| English | Chinese |
|---|---|
| boars | 野猪 |
| champions | 骑兵 |
| deck of unit cards | 单位牌堆 |
| dendroids | 枯木卫士 |
| dread knights | 死亡骑士 |
| familiars | 怪物 |
| few | 少量 |
| griffins | 狮鹫 |
| hp | 生命值 |
| liches | 尸巫 |
| marksmen | 神射手 |
| neutral units | 中立单位 |
| pack | 一群 |
| recruited or reinforced | 被招募或增援 |
| reinforce | 增援 |
| reinforced | 已增援 |
| summoned elementals | 召唤元素 |
| unit cards | 单位牌 |
| unit miniatures | 单位模型 |
| units | 单位 |
| zealots | 祭司 |

## Heroes

| English | Chinese |
|---|---|
| MP | 移动点 |
| ability | 能力 |
| experience | 经验 |
| expert effects | 专家效果 |
| hero board | 英雄板 |
| hero | 英雄 |
| heroes | 英雄 |
| level tracker | 等级轨道 |
| level | 等级 |
| main hero | 主英雄 |
| movement points | 移动点 |
| MP | 移动点 |
| power | 魔法威力 |
| secondary hero | 副英雄 |
| specialty | 特长 |

## Map Locations

| English | Chinese |
|---|---|
| center field | 中央格 |
| central map tile | 中央地图板块 |
| creature bank | 怪物宝屋 |
| dragon utopia | 龙之国 |
| far map tile | 远端地图板块 |
| field | 格 |
| locations | 地点 |
| map tile | 地图板块 |
| mines | 矿场 |
| monolith | 传送门 |
| near map tile | 近端地图板块 |
| obelisk | 方尖碑 |
| random town | 随机城镇 |
| revisitable | 可重复访问 |
| sea map tile | 海洋地图板块 |
| settlement | 定居点 |
| starting (i) map tile | 起始（I）地图板块 |
| subterranean gate | 地下通道 |
| subterranean map tile | 地下地图板块 |
| tile | 板块 |
| trading post | 贸易站 |
| water wheel | 水车 |
| whirlpool | 漩涡 |

## Resources

| English | Chinese |
|---|---|
| building materials | 建筑材料 |
| gold | 金币 |
| grail token | 圣杯标记 |
| resource dice | 资源骰 |
| resource production tracker | 资源生产轨道 |
| resources | 资源 |
| treasure die | 宝物骰 |
| valuables | 贵重物品 |

## Deck-building

| English | Chinese |
|---|---|
| ability card | 能力牌 |
| deck | 牌堆 |
| decks | 牌堆 |
| discard pile | 弃牌堆 |
| discard | 弃置 |
| empowered ability card | 强化能力牌 |
| empowered ability token | 强化能力标记 |
| empowered statistic card | 强化属性牌 |
| hand limit | 手牌上限 |
| pandora's box card | 潘多拉魔盒牌 |
| player decks | 玩家牌堆 |
| search | 搜寻 |
| specialty card | 特长牌 |
| spell cards | 法术牌 |
| spell scroll | 法术卷轴 |
| statistic card | 属性牌 |
| summoned unit card | 召唤单位牌 |

## Spells

| English | Chinese |
|---|---|
| haste | 加速术 |
| magic arrow | 魔法箭 |
| school | 派系 |

## AI Deck

| English | Chinese |
|---|---|
| ai deck | AI牌堆 |
| magic card | 魔法牌 |
| might card | 力量牌 |
| skill card | 技能牌 |

## Town

| English | Chinese |
|---|---|
| build token | 建造标记 |
| buildings | 建筑 |
| citadel | 大本营 |
| dwelling | 兵营 |
| population token | 人口标记 |
| spell book token | 法术书标记 |
| town board | 城镇板 |
| town card | 城镇牌 |

## Combat

| English | Chinese |
|---|---|
| arrow tower | 箭塔 |
| back line | 后排 |
| combat board | 战斗板 |
| combat | 战斗 |
| damage | 伤害 |
| elemental damage | 元素伤害 |
| front line | 前排 |
| initiative | 先攻 |
| negative morale | 低落士气 |
| positive morale | 高昂士气 |
| winning combat | 赢得战斗 |

## Battlefield

| English | Chinese |
|---|---|
| adventure card | 冒险牌 |
| adventure mode | 冒险模式 |
| alliance | 同盟 |
| battlefield board | 战场板 |
| battlefield obstacles | 战场障碍物 |
| battlefield | 战场 |
| campaign | 战役 |
| clash | 冲突 |
| combat power level | 战力等级 |
| cooperative | 合作 |
| final combat | 最终战斗 |
| scenario | 剧本 |
| skirmish mode | 遭遇战模式 |

## Keywords

| English | Chinese |
|---|---|
| attack token | 攻击标记 |
| clone token | 克隆标记 |
| corrosion token | 腐蚀标记 |
| fire wall token | 火墙标记 |
| force field token | 力场标记 |
| land mine token | 地雷标记 |
| miniatures | 模型 |
| quicksand token | 流沙标记 |
| stack token | 堆叠标记 |
| summon token | 召唤标记 |
| time token | 时间标记 |
| weakness token | 虚弱标记 |

## Other

| English | Chinese |
|---|---|
| artifact | 神器 |
| astrologers proclaim card | 星象预言牌 |
| attack dice | 攻击骰 |
| black cubes | 黑色方块 |
| core rulebook | 核心规则书 |
| effects | 效果 |
| event card | 事件牌 |
| expert effect markers | 专家效果标记 |
| faction cubes | 阵营方块 |
| faction | 阵营 |
| game setup | 游戏准备 |
| map setup | 地图准备 |
| player elimination | 玩家淘汰 |
| player setup | 玩家准备 |
| relic | 圣物 |
| round | 轮 |
| round tracker | 轮次轨道 |
| turn | 回合 |
| war machine | 战争机器 |
